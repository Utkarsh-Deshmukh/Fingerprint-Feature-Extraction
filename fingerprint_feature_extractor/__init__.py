...
class FingerprintFeatureExtractor(object):
		...
		def processImage(self, FeaturesTerm, FeaturesBif, showResult, saveResult):
				...
				if saveResult:
					cv2.imwrite(saveResult, DispImg)


def extract_minutiae_features(img, showResult=False, saveResult=None):
		feature_extractor = FingerprintFeatureExtractor()
		FeaturesTerm, FeaturesBif = feature_extractor.extractMinutiaeFeatures(img)

		if saveResult or showResult:
			feature_extractor.processImage(FeaturesTerm, FeaturesBif, showResult, saveResult)
